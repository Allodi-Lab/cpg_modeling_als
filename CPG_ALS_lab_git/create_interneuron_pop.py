...
class interneuron_population():
    ...
    def create_interneuron_population(self,pop_type,self_connection,firing_behavior,pop_size,input_type):
        ...
        if firing_behavior == 'tonic':
        ...
        elif firing_behavior == 'bursting' and pop_type == 'rc_1':
            ...
            self.bursting_neuronparams = {'C_m':nest.random.normal(mean=netparams.C_m_bursting_mean, std=netparams.C_m_bursting_std), 'g_L':26.,'E_L':-60.,'V_th':nest.random.normal(mean=netparams.V_th_mean_bursting, std=netparams.V_th_std_bursting),'Delta_T':2.,'tau_w':130., 'a':-11., 'b':30., 'V_reset':-48., 'I_e':self.I_e,'t_ref':nest.random.normal(mean=netparams.t_ref_mean, std=netparams.t_ref_std),'V_m':nest.random.normal(mean=netparams.V_m_mean, std=netparams.V_m_std),"tau_syn_rise_I": netparams.tau_syn_i_rise, "tau_syn_decay_I": netparams.tau_syn_i_decay, "tau_syn_rise_E": netparams.tau_syn_e_rise_rc, "tau_syn_decay_E": netparams.tau_syn_e_decay_rc} #bursting, Naud et al. 2008, C = pF; g_L = nS    
        ...
        elif firing_behavior == 'bursting' and pop_type == 'rc_2':
            self.I_e = nest.random.normal(mean=netparams.I_e_bursting_mean, std=netparams.I_e_bursting_std) if input_type=='descending' else nest.random.normal(mean=netparams.I_fb_bursting_mean, std=netparams.I_fb_bursting_std) if input_type=='sensory_feedback' else 0
            ...
            self.interneuron_pop = nest.Create('aeif_cond_beta_aeif_cond_beta_nestml',pop_size,self.bursting_neuronparams)
            self.white_noise = nest.Create("noise_generator",netparams.noise_params_bursting)    
        #print(nest.GetStatus(self.interneuron_pop, "I_e"))
